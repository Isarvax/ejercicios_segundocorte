/**
 * Represents the seating arrangement in sala1.
 * @type {boolean[]}
 */
const sala1 = Array(10).fill(false);
/**
 * Represents the availability of seats in sala2.
 * @type {boolean[]}
 */
const sala2 = Array(10).fill(false);

// Cost of each ticket in pesos
const costoBoleto = 100;  // Assuming a cost of 100 pesos per ticket

/**
 * Displays the availability of seats in two different rooms.
 */
function verDisponibilidad() {
  alert("Sala 1:");
  alert(sala1.map((asiento, i) => asiento ? `[X] Asiento ${i + 1}` : `[ ] Asiento ${i + 1}`).join("\n"));
  alert("Sala 2:");
  alert(sala2.map((asiento, i) => asiento ? `[X] Asiento ${i + 1}` : `[ ] Asiento ${i + 1}`).join("\n"));
}

/**
 * Reserves a seat in a movie theater.
 * @function reservarAsiento
 */
function reservarAsiento() {
  const sala = prompt("Ingresa el número de la sala (1 o 2)");
  const asiento = parseInt(prompt("Ingresa el número del asiento (1 a 10)")) - 1;

  if (sala === "1" && !sala1[asiento]) {
    sala1[asiento] = true;
    alert(`Asiento ${asiento + 1} de la sala 1 reservado.`);
  } else if (sala === "2" && !sala2[asiento]) {
    sala2[asiento] = true;
    alert(`Asiento ${asiento + 1} de la sala 2 reservado.`);
  } else {
    alert("El asiento ya está reservado o la sala no es válida.");
  }
}

/**
 * Verifies the availability of seats in a movie theater based on the selected room.
 */
function verPelicula() {
  const sala = prompt("Ingresa el número de la sala (1 o 2)");
  const asientosLibres = sala === "1" ? sala1.filter(asiento => !asiento) : sala2.filter(asiento => !asiento);

  if (asientosLibres.length > 0) {
    alert(`Hay ${asientosLibres.length} asientos libres en la sala ${sala}.`);
    alert("Disfrutad de la película!");
  } else {
    alert(`Lo siento, todos los asientos de la sala ${sala} están reservados.`);
  }
}

/**
 * Displays the total money collected from both rooms.
 */
function verBalance() {
  const totalSala1 = sala1.filter(asiento => asiento).length * costoBoleto;
  const totalSala2 = sala2.filter(asiento => asiento).length * costoBoleto;
  const totalGeneral = totalSala1 + totalSala2;

  alert(`Total recaudado en Sala 1: $${totalSala1}`);
  alert(`Total recaudado en Sala 2: $${totalSala2}`);
  alert(`Total recaudado general: $${totalGeneral}`);
}

// Principal Menu
while (true) {
  console.log("Hola, bienvenido a la sala de cine");
  const opcion = prompt("Selecciona una opción:" + 
  "\n1. Ver disponibilidad de asientos" + 
  "\n2. Reservar asiento" + 
  "\n3. Ver ocupación de la sala" + 
  "\n4. Ver balance de recaudación" +
  "\n5. Salir");

  switch (opcion) {
    case "1":
      verDisponibilidad();
      break;
    case "2":
      reservarAsiento();
      break;
    case "3":
      verPelicula();
      break;
    case "4":
      verBalance();
      break;
    case "5":
      alert("¡Hasta luego!");
      break;
    default:
      alert("Opción no válida. Intenta de nuevo.");
  }
}
